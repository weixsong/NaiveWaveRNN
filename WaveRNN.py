...
class WaveRNN_Alternative(object):
    def __init__(self, is_training=True):
        ...
        self.aux_lc_partial_dim = hp.lc_dims // 4
        ...
        with tf.variable_scope('WaveRNN', reuse=tf.AUTO_REUSE):
            self.upsample_conv_in = tf.get_variable('upsample_conv_in', [hp.conv_in_width, hp.num_mels, hp.lc_dims], initializer=initializer)
            self.upsample_filters = []
            for i, stride in enumerate(hp.upsample_factors):
                filter_width = hp.upsample_widths[i]
                filter = tf.get_variable('upsample_filter_{}'.format(i), [stride * filter_width, hp.lc_dims, hp.lc_dims], initializer=initializer)
                self.upsample_filters.append(filter)

            # auxiliary local condition
            self.aux_conv_in = tf.get_variable('aux_conv_in', [hp.conv_in_width, hp.num_mels, hp.lc_dims], initializer=initializer)
            self.aux_conv_out = tf.get_variable('aux_conv_out', [1, hp.lc_dims, hp.lc_dims], initializer=initializer)
            self.aux_conv_out_bias = tf.get_variable('aux_conv_out_bias', [hp.lc_dims], initializer=bias_initializer)
            self.aux_in_bn = tf.layers.BatchNormalization(name='aux_in_bn')
            self.aux_res = {}
            for i in range(hp.res_blocks):
                res_blocks = []
                conv1_params = tf.get_variable('res_{}_conv1'.format(i), [1, hp.lc_dims, hp.lc_dims], initializer=initializer)
                batch_norm1 = tf.layers.BatchNormalization(name='res_{}_bn1'.format(i))
                conv2_params = tf.get_variable('res_{}_conv2'.format(i), [1, hp.lc_dims, hp.lc_dims], initializer=initializer)
            ...
            # linear layers
            self.fc_1 = tf.get_variable('fc_1', [1 + hp.lc_dims + self.aux_lc_partial_dim, hp.rnn_dims], initializer=initializer)
            self.bias_fc_1 = tf.get_variable('bias_fc_1', [hp.rnn_dims], initializer=bias_initializer)

            self.fc_2 = tf.get_variable('fc_2', [hp.rnn_dims + self.aux_lc_partial_dim, hp.fc_dims], initializer=initializer)
            self.bias_fc_2 = tf.get_variable('bias_fc_2', [hp.fc_dims], initializer=bias_initializer)

            self.fc_3 = tf.get_variable('fc_3', [hp.fc_dims + self.aux_lc_partial_dim, hp.fc_dims], initializer=initializer)
    ...
    def build_network(self, x, y, mel):
        ...
        input = tf.reshape(input, [-1, 1 + hp.lc_dims + self.aux_lc_partial_dim])
        ...
        x = tf.reshape(x, [-1, hp.rnn_dims + self.aux_lc_partial_dim])
        ...
        x = tf.reshape(x, [-1, hp.fc_dims + self.aux_lc_partial_dim])
    ...
    def upsample_network(self, mel):
        ...
        aux = tf.reshape(mel, [batch_size, -1, hp.lc_dims])             # B*1800*128
        ...
        for i, stride in enumerate(hp.upsample_factors):
            ...
            lc_shape = tf.shape(mel)
            batch_size, lc_length, lc_dim = lc_shape[0], lc_shape[1], lc_shape[2]
            output_shape = [batch_size, lc_length * stride, hp.lc_dims]
            mel = tf.contrib.nn.conv1d_transpose(mel, filter, output_shape, stride=stride)
            mel = tf.nn.relu(mel)
